calcShapes = function (verts) {
	...
	while (queue.length) {
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		vec = queue[0]; n = vec.length; isConvex = true///
		_.t(n, function (i) {i1 = i; i2 = (i < n - 1) ? i + 1 : i + 1 - n; i3 = (i < n - 2) ? i + 2 : i + 2 - n
			p1 = vec[i1];p2 = vec[i2];p3 = vec[i3]
			if ((d = det(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y)) < 0) {
				//////
				
				
				
			
				isConvex = false; minLen = Number.MAX_VALUE
				
				_.t(n, function (j) {if (j != i1 && j != i2) {
					j1 = j; j2 = (j < n - 1) ? j + 1 : 0; v1 = vec[j1];v2 = vec[j2];
				...
				while (true) {
					if (k == j2) {if (!isOnSegment(v2.x, v2.y, vec[h].x, vec[h].y, p1.x, p1.y)) {
				...
				while (true) {
					if (k == j1) {if (k == j1 && !isOnSegment(v1.x, v1.y, vec[h].x, vec[h].y, p2.x, p2.y)) {
