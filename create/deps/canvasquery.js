(function(window, undefined) {
    ...
    var $ = function(selector) {
        ...
        else if (selector instanceof $.Wrapper){return selector}
    ...
    $.Wrapper.prototype = {
        ...
        appendTo: function(selector) {
            if (typeof selector === "object"){var element = selector}

            else {var element = document.querySelector(selector)}
        ...
        colorToMask: function(color, inverted){
            ...
            var sourceData = this.context.getImageData(0, 0,
        ...
        fillStyle: function(fillStyle) {  this.context.fillStyle = fillStyle;  return this},
    ...
    var methods = ["arc", "arcTo", "beginPath", "bezierCurveTo", "clearRect", "clip", "closePath",
        "createImageData", "createLinearGradient", "createRadialGradient", "createPattern",
        "drawFocusRing", "drawImage", "fill", "fillRect", "fillText", "getImageData", "isPointInPath",
        "lineTo", "measureText", "moveTo", "putImageData", "quadraticCurveTo", "rect", "restore",
        "rotate", "save", "scale", "setTransform", "stroke", "strokeRect", "strokeText",
        "transform", "translate"]
    ...
    for (var i = 0; i < methods.length; i++) {
        ...
        if ( !$.Wrapper.prototype[name] ){

            var str="this.context." + name   + ".apply(this.context, arguments); return this"

            $.Wrapper.prototype[name] = Function(str)
    ...
    var properties = [
        ...
        "canvas", "fillStyle", "font", "globalAlpha", "globalCompositeOperation", "lineCap",
        "lineJoin", "lineWidth", "miterLimit", "shadowOffsetX", "shadowOffsetY", "shadowBlur",
        "shadowColor", "strokeStyle", "textAlign", "textBaseline"
    ]
    ...
    for (var i = 0; i < properties.length; i++){
        ...
        var str="if(arguments.length) { this.context." + name
            + " = arguments[0]; return this; } else { return this.context." + name + "; }"

        if (!$.Wrapper.prototype[name]) $.Wrapper.prototype[name] = Function(str)
    }
...
CANVASQ=function(){
    ...
    var player = {
        x: 200,        // Initial position.x of the shape
        y: 200,        // Initial position.y of the shape
        rotation: 0,
        speed: 0.01,   // How fast it rotates
        width: 100,    // Width of the shape
        height: 100    // Height of the shape
    }


    var distanceFromCursor = 20
    ...
    var fps = 0,
        ...
        x = document.getElementById("ShowFPS")
    ...
    cq().framework({
        ...
        onmousemove: function(x, y) {
            isMouseMoving = true;
            player.x = x + player.width/2 + distanceFromCursor;
            player.y = y + player.height/2 + distanceFromCursor;
            d = new Date();
            oldTime = d.getTime()},
        ...
        onstep: function(delta) {
            ...
            if (isMouseMoving || (timeSpan < animationDelay)) {
                player.rotation += 0
            }
            else {
                if ((!isMouseMoving) && (timeSpan < animationDelay*5))
                    player.rotation += player.speed/3;
                else
        ...
        onrender: function() {
            this.save()
                ...
                .fillStyle("rgba(32, 64, 128, 1)")
                .roundRect(-(player.width / 2) + distanceFromCursor,
                    -(player.height / 2) + distanceFromCursor,
                player.width, player.height, shapeRadius)
                .fill()
                .fillStyle("rgba(255, 180, 0, 0.7)")
                .roundRect(-player.width / 2, -player.height / 2,
                player.width, player.height, shapeRadius)
                .fill()
                .restore()
            fps++
