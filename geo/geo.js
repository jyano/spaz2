gpc = gpcas; gpc.g = gpc.geometry; b2d.p(); ps = gpc.g.PolySimple.prototype; pD = gpc.g.PolyDefault.prototype; h = cjs.Shape.prototype
$vs = {}
...
$UNI= $uni = UNI = b2d.u = function me() {var g = G(arguments)
	
	if (g.A) {return g.ap($UNI)}//<<return $a(UNI,g.f)
	var p = M.p(g.f)
	g.eR(function (pol) {
		pol= M.p(pol)
		p = p.U(pol)
...
b.scrapeEachF = b.subAll = function (pol) {
...
b.subPol = b.subPoly = b.subP = function (pol, xy) {
	pol = $pol(pol, xy)
	return this.scrapeEachF(pol)
}
...
b.expl = function (c) {
	...
	return b.cl(function (f) {
		...
		b.subPolAtFxt('expl', f, '-')._colorize(c)
	})
}
function oldDIf() {
...
U1 = function () {W().G(1000).roof.kill()
	b = w.S(100, 400, 'r', [-40, -20], [0, -100], [50, 25])
	RED  = b.f()
	ORANGE = w.S(150, 420, 'o', 50, 50) 
	orange = ORANGE.f()
	YELLOW = w.S(100, 340, 'y', 50, 50)
	yellow= YELLOW.f()
	
	w.D(200, 400).pol({v: RED.uni(orange)}) 
	
	w.D(350, 400).pol(RED.uni(yellow)) 
	
	w.D(440, 400).pol('x', RED.uni(orange, yellow)) 
	
	w.D(300, -200).pol( yellow.uni(orange) ) 
...
U2=function(){W()
	...
	b1= w.S(200, 200, [['r', 50, 50], ['o', 10, 100]]).uni('+')
