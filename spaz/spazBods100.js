...
b.cF= b._f = function () {var b = this, g = G(arguments),o
	//pass fD,color  .... OR klass,fD,color
	//var iFD=b2d.iFD

	o = g.O? g.f: 
			g.S_? {k: g.f, fD: g.s, c: g.t}:
			 {fD: g.f, c: g.s}
	...
	var arr
	 ...
	 if (g.SA) {
		 ...
		 g.e1(function (f) {//       'c',  [ [.,.], fD, [.,.] ]
			 if (b2d.iFD(f)) {   b.cF(f, COLOR)   }
			 else {
				 arr = f
				 // we have an array
				 // if it doesnt START with a string (which reps color)..
				 // then give it default color
				 if (!S( _.f(arr) )) { arr.unshift( COLOR ) }
				 /// ok whatever, now we have ...
				 // [   colorString,   anotherArray ] 
				 //  b2d.iFD(arr[1]) ? b.cF(arr[1], arr[0]) :
				 $a(b, 'f', arr)
			 }
		 })
	 }
